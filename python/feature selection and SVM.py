...
'''
y_pred1 = np.argmax(prob,axis = 1)
y_pred2 = np.argmax(prob1,axis = 1)
'''
...
'''
# transition matrix using scalp EEG hyponogram
N_next_state = np.zeros((4,4))
for kk in range(4):    # 4 data files
    y_num = y[kk]   # choose an EEG data file
    y_cut = y[kk][0:len(y_num)-1]   # the last state doesn't have next state    
    for pre in range(4):         # 4 previous states
        num1 = np.where(y_cut == pre)[0]  # after cut (labels of the previous state)
        next_state = y_num[num1+1]        
        for ii in range(4):
...
y_cut = label_total[0:len(label_total)-1]
N_next_state = np.zeros((4,4))
for pre in range(4):
    num1 = np.where(y_cut == pre)[0]
    next_state = label_total[num1+1]
    for ii in range(4):
        N_next_state[pre,ii] = N_next_state[pre,ii] + sum(next_state == ii)
...
'''
# transition matrix in paper
transition_matrix = np.zeros((4,4))
transition_matrix[0,0] = 0.8624
transition_matrix[0,1] = 0.0811
transition_matrix[0,2] = 0.0467
transition_matrix[0,3] = 0.0015
transition_matrix[1,0] = 0.0891
transition_matrix[1,1] = 0.5730
transition_matrix[1,2] = 0.2809
transition_matrix[1,3] = 0.0001
transition_matrix[2,0] = 0.0289
transition_matrix[2,1] = 0.0001
transition_matrix[2,2] = 0.9284
transition_matrix[2,3] = 0.0333
transition_matrix[3,0] = 0.0130
transition_matrix[3,1] = 0
transition_matrix[3,2] = 0.0639
transition_matrix[3,3] = 0.9227
'''
...
label_previous = label_total[test_idx_previous]
